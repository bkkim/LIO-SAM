...
public:
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr laserCloudMlx; // cloudInfo.cloud (added by kbk)
    ...
    ~mapOptimization()
    {
        ROS_INFO("!!!destructor test!!!");
        if (saveKeyframeFlag == true)
        {       
            int count_cloud = cornerCloudKeyFrames.size();
            int count_pose  = cloudKeyPoses6D->size();
            ROS_INFO("count_cloud : %d, count_pose : %d", count_cloud, count_pose);

            for (size_t i = 0; i < count_pose; i++)
            {                
                pcl::PointCloud<PointType>::Ptr transform_cloud(new pcl::PointCloud<PointType>());    
                *transform_cloud += *transformPointCloud(surfCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
                *transform_cloud += *transformPointCloud(cornerCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
                // save keyframes
    ...
    void allocateMemory()
    {
        ...
        laserCloudMlx.reset(new pcl::PointCloud<pcl::PointXYZRGB>()); // original point cloud (added by kbk)
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        pcl::fromROSMsg(msgIn->cloud, *laserCloudMlx);
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            totalTime += duration;
            totalCnt ++;
            auto avg = std::chrono::milliseconds(totalTime.count()/totalCnt);
            if (moTime)
                std::cout << "mapOptimization::laserCloudInfoHandler() : " << duration.count() << " ms" << ", Avg : " << avg.count() << " ms" << std::endl;
        }
    ...
    {
        if (placeRecognitionEnableFlag == false)
            return;
        ...
        {
            rate.sleep();
            performPlaceRecognition();
        }
    }

    void performPlaceRecognition() 
    {
        if (cloudKeyPoses3D->points.empty() == true)
            return;
        mtx.lock();
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        if (detectPrevMapDBow(&prKeyCur, &prKeyPre, &score) == false)
            return;
        ...
        {
            prFindNearKeyframes(curKeyframeCloud, prKeyCur, 0);
            prFindNearKeyframes(preKeyframeCloud, prKeyPre, historyKeyframeSearchNum);
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        int loopKeyPre;
        float score;
        if (loopClosureType == 1) // DBoW
        { 
            // detect by DBow (kbk)
            if (detectLoopClosureDBow(&loopKeyCur, &loopKeyPre, &score) == false)   
                return;
        } 
        else 
        {
        ...
        if (loopKeyPre == 0 || loopKeyPre == 1)
        {
            if (score > 0.7)
            {
                // Save local map.
            
                pcl::io::savePCDFileASCII ("/home/wave/2025_workspace/test_liosam_ws/local_map.pcd", *laserCloudSurfFromMapDS);
                cout << "slam end!!" << endl;
                ros::shutdown();
            } 
        }
        ...
        // GICP alignment (both clouds are already in world frame, identity initial guess is correct)
        ...
        if (pubIcpKeyFrames.getNumSubscribers() != 0)
        {
            ...
            // save KeyframeCloud (kbk)
            if (saveIcpframeFlag)
            {
                // save keyframes
                std::string filename = saveDataPath + "currkeyframe_" + std::to_string(loopKeyCur) + ".pcd";
                pcl::io::savePCDFileASCII (filename, *cureKeyframeCloud);
                filename = saveDataPath + "prevkeyframe_" + std::to_string(loopKeyCur) + "_" + std::to_string(loopKeyPre) + ".pcd";
                pcl::io::savePCDFileASCII (filename, *prevKeyframeCloud);
                filename = saveDataPath + "transkeyframe_" + std::to_string(loopKeyCur) + ".pcd";
                pcl::io::savePCDFileASCII (filename, *closed_cloud);
            }
        }
        ...
        // kbk
        //cout << "noiseScore : " << noiseScore << endl;
        //if (0) { Vector6 << 1e-5, 1e-5, 1e-5, 1e-6, 1e-6, 1e-6; }
        ...
        if (loopClosureType == 1 && saveDBoWFlag == true)
	    {
            // Query image
            char szBuf[128];
            sprintf(szBuf, "%04d", loopKeyCur);
            std::string filename = saveDataPath + "keyFrame_" + std::string(szBuf) + ".png";
            cv::Mat img_test = cv::imread(filename);
	    	cv::putText(img_test, "Curr File: " + std::string(szBuf), cv::Point2f(5, 10), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 0, 255), 1);
	    	
	    	// DB image
            sprintf(szBuf, "%04d", loopKeyPre);
            filename = saveDataPath + "keyFrame_" + std::string(szBuf) + ".png";
	    	cv::Mat img_db = cv::imread(filename);
	    	std::string caption = "Prev File: " + std::string(szBuf) + ", Score: " + to_string(score);
	    	cv::putText(img_db, caption, cv::Point2f(5, 10), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 0, 255), 1);
	    	cv::Mat img_gap(2, img_db.size().width, CV_8UC3, cv::Scalar(255, 255, 255));
	    		
	    	cv::vconcat(img_test, img_gap, img_gap);
	    	cv::vconcat(img_gap, img_db, img_db);
	    	cv::imshow("Search image", img_db);

	    	sprintf(szBuf, "%04d_%04d", loopKeyCur, loopKeyPre);
            filename = saveDataPath + "Loop_" + std::string(szBuf) + ".png";
	    	cv::imwrite(filename, img_db);
	    }
    }

    bool detectLoopClosureDBow(int *latestID, int *closestID, float *score)
    {
        int loopKeyCur = copy_cloudKeyPoses3D->size() -1;
        int loopKeyPre = -1;

        if (keyFrameDescriptors.size() < keyframeSearchGap + 1 /*minimum gap*/)
            return false;

        QueryResults results;
    	dbowDb.query(keyFrameDescriptors[loopKeyCur], results, 1);
    	cout << "Searching for KeyFrame " << loopKeyCur << ". " << results << endl;

        if (results.size() == 0 || results[0].Score < dbowMinScore /*0.5*/)
            return false;
        else 
        {
            PointType pt_c = copy_cloudKeyPoses3D->points[loopKeyCur];
            PointType pt_p = copy_cloudKeyPoses3D->points[results[0].Id];
            float distance = pointDistance(pt_c, pt_p);
            if (distance > keyframeMaxDist /*3*/) {
                return false;
            }
            cout << "Query File: " << loopKeyCur << ", Search File: " << to_string(results[0].Id) << " (" << to_string(results[0].Score) << ")" << endl;
	    ...
	    if (0/*DEBUG_IMAGE*/)
	    {
	    	...
	    	{
	    		// Image load
                sprintf(szBuf, "%04d", results[0].Id);
                filename = saveDataPath + "keyFrame_" + std::string(szBuf) + ".png";                

	    		cv::Mat img_db = cv::imread(filename);
	    		std::string caption = "Prev File: " + std::string(szBuf) + ", Score: " + to_string(results[j].Score);
	    		cv::putText(img_db, caption, cv::Point2f(5, 10), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 0, 255), 1);
	    		cv::Mat img_gap(2, img_db.size().width, CV_8UC3, cv::Scalar(255, 255, 255));
	    		...
	    		sprintf(szBuf, "%04d_%04d", loopKeyCur, results[0].Id);
                filename = saveDataPath + "Loop_" + std::string(szBuf) + ".png";
	    		cv::imwrite(filename, img_db);
	    	//	cv::waitKey(10);
	    	}
    ...
    void extractNearby()
    {
        ...
        // also extract some lastest key frames in case the robot rotates in one position
        int numPoses = cloudKeyPoses3D->size();
        for (int i = numPoses-1; i >= 0; --i)
        {
    ...
    bool saveFrame()
    {
        ...
        if (sensor == SensorType::LIVOX || keyframeType == 1)
        {
...
*/
    ...
    void tryRelocalize()
    {
        if (!laserCloudMlx || laserCloudMlx->empty()) 
            return;
        ...
        *currentScan += *laserCloudSurfLastDS;
        ...
        { 
            ROS_WARN("[Relocalize] currentScan too small (<300), skip"); 
            return; 
        }
        ...
        { 
            ROS_WARN("[Relocalize] refCloud too small (<1000), skip"); 
            return; 
        }
        ...
        // kbk 20260706
        // Map based place recognition thread start!
        std::thread prthread(&mapOptimization::placeRecognitionThread, this);
    }
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        // kbk
        //Pose3 firstFrame = isamCurrentEstimate.at<Pose3>(0);
        //ROS_INFO("1st Keyframe : %f, %f, %f", firstFrame.x(), firstFrame.y(), firstFrame.z());
        double x = thisPose6D.x;
        double y = thisPose6D.y;
        double z = thisPose6D.z;
        double dist = sqrt((x*x) + (y*y) + (z*z));
        if (odomInfo)
        ROS_INFO("%d Keyframe pose : %f, %f, %f, (%f)", isamCurrentEstimate.size(), x, y, z, dist);

        // kbk
        //if (loopClosureEnableFlag && loopClosureType == 1)
        if (loopClosureType == 1)
        {
            keyFrameMlx.push_back(laserCloudMlx);

            size_t width = laserCloudMlx->width;
            size_t height = laserCloudMlx->height;

            cv::Mat rangeMat;
            rangeMat = cv::Mat(height-1 , width-1, CV_32F, cv::Scalar::all(FLT_MAX));
            ...
            for (size_t v = 0; v < height-1; v++)
            {
                for (size_t u = 0; u < width-1; u++)
                {
                    ...
                    pcl::PointXYZRGB pt_c = laserCloudMlx->points[(v  )*width+(u  )];
                    pcl::PointXYZRGB pt_r = laserCloudMlx->points[(v  )*width+(u+1)];
                    pcl::PointXYZRGB pt_b = laserCloudMlx->points[(v+1)*width+(u  )];

                    float pt_c_range = range(pt_c);
                    float pt_r_range = range(pt_r);
                    float pt_b_range = range(pt_b);

                    if (pt_c_range != 0 && pt_r_range != 0 && pt_b_range != 0)
                    {
                        float diff_h = pt_c_range - pt_r_range;
                        float diff_v = pt_c_range - pt_b_range;
                        if (abs(diff_h) > thres_h && abs(diff_v) > thres_v)
                        {
                            diffs.push_back(pt_c_range);
                            diffs.push_back(diff_h);
                            diffs.push_back(diff_v);
                        }
                    }

                    rangeMat.at<float>(v, u) = pt_c_range*25;
                }
            }
        ...
        // Added by kbk
        if (0/*saveKeyframeFlag == true*/)
        {
            pcl::PointCloud<PointType>::Ptr transform_cloud(new pcl::PointCloud<PointType>());    
            *transform_cloud += *transformPointCloud(thisCornerKeyFrame, &thisPose6D);
            *transform_cloud += *transformPointCloud(thisSurfKeyFrame, &thisPose6D);
            // save keyframes

            char szBuf[128];
            sprintf(szBuf, "%04d", cornerCloudKeyFrames.size());
            std::string filename = saveDataPath + "KeyPointCloud_" + std::string(szBuf) + ".pcd"; 
            pcl::io::savePCDFileASCII (filename, *transform_cloud);
        }
        ...
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        if (moTime)
        std::cout << "   mapOptimization::saveKeyframesAndFactor() : " << duration.count() << " ms" << std::endl;
    }
    ...
    void publishOdometry()
    {
        ...
        double dist = sqrt((x*x) + (y*y) + (z*z));
        if (odomInfo)
        ROS_INFO("publishOdometry : %f, %f, %f, (%f)", x, y, z, dist);
